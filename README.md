...
- [Docker Compose 볼륨 자동 재등록](#docker-compose-볼륨-자동-재등록)
...
| `COMPOSE_FILE`  | (없음)                                   | `docker-compose.yml` 경로. 지정하면 Compose-managed 볼륨 마이그레이션 후 `docker compose up --no-start`를 자동 실행하여 Compose 메타데이터를 재등록                                    |
...
# Compose 볼륨 자동 재등록 포함
sudo COMPOSE_FILE=/opt/myapp/docker-compose.yml ./userns_migration.sh
```
...
### Phase 5b — Compose-managed 볼륨 재등록

Phase 5 직후, 마이그레이션된 볼륨 중 `com.docker.compose.project` 레이블을 가진 볼륨(= Compose가 생성한 볼륨)을 감지합니다.

**왜 이 단계가 필요한가?**

Phase 5에서 `docker volume create`로 볼륨을 등록하면 Docker 데몬은 볼륨을 인식하지만, Compose는 자신이 생성하지 않은 볼륨에 대해 다음 오류를 냅니다.

```
Error: volume 'myapp_data' was not created by Compose
```

`docker compose up --no-start`를 실행하면 컨테이너를 시작하지 않고 Compose가 볼륨 메타데이터만 재등록합니다. 이미 `_data`에 복사된 데이터는 그대로 유지됩니다.

**`COMPOSE_FILE` 설정 여부에 따른 동작:**

| 상황                                                        | 동작                                                      |
| ----------------------------------------------------------- | --------------------------------------------------------- |
| `COMPOSE_FILE=/path/to/docker-compose.yml` 지정 (파일 존재) | `docker compose -f $COMPOSE_FILE up --no-start` 자동 실행 |
| `COMPOSE_FILE` 지정했으나 파일 없음                         | 경고 출력 후 건너뜀                                       |
| `COMPOSE_FILE` 미지정                                       | Compose-managed 볼륨 목록과 수동 실행 명령어를 출력       |

**`COMPOSE_FILE` 미지정 시 출력 예시:**
...
Compose-managed 볼륨이 없으면 이 단계는 자동으로 건너뜁니다.
...
### 2. Compose 볼륨 재등록 (Compose 사용 시)

`COMPOSE_FILE`을 지정하지 않았거나 스크립트가 자동 재등록을 건너뛴 경우, Compose 프로젝트 디렉토리에서 실행합니다.

```bash
docker compose up --no-start
```

이 명령은 컨테이너를 시작하지 않고 Compose가 볼륨 메타데이터만 재등록합니다. **이 단계를 건너뛰면 `docker compose up -d` 시 "volume was not created by Compose" 오류가 발생할 수 있습니다.**

> `COMPOSE_FILE`을 지정하여 스크립트가 자동으로 처리한 경우에는 이 단계를 건너뛰어도 됩니다.
